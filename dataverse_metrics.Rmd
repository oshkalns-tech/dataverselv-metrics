...
autoplot(stats_ts, facets = FALSE) +  labs(title="Statistics", color="Statistic") +
    scale_y_continuous(labels=scales::comma, breaks = seq(0, max(stats_ts, na.rm = TRUE), by = 2))





filedownloads <- content(GET(paste0("https://", dataverse_host,
                                    "/api/info/metrics/filedownloads/monthly")))
filedownloads <- bind_rows(filedownloads$data)

stats_ts <- ts(filedownloads[, -1], frequency = 12,
               start = strsplit(filedownloads$date[1], split = "-")[[1]])

autoplot(stats_ts, facets = FALSE) +
  labs(title = "File Downloads Statistics", color = "Statistic") +
  scale_y_continuous(labels = scales::comma,
                     breaks = seq(0, max(stats_ts, na.rm = TRUE), by = 2))
